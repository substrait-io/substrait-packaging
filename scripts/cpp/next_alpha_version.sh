...
PACKAGE="$1"   # e.g. substrait-protobuf, substrait-antlr, substrait-extensions
VERSION="$2"
...
# For tag-as-package C++ artifacts a pushed version tag *is* the publish, and
# there is no registry/proxy, so the repository's own tags are the sole source
# of truth for which alpha pre-releases already exist. The caller must have
# fetched tags (the workflows check out with fetch-depth: 0).
#
...
PREFIX="cpp/$PACKAGE/v${VERSION}-alpha"
MAX=-1
