...
Versions of this artifact correspond to Substrait [releases](https://github.com/substrait-io/substrait/releases). `vx.y.z` of substrait-antlr will contain code generated from `vx.y.z` of the [substrait repository](https://github.com/substrait-io/substrait).
...
```
io.substrait:substrait-antlr:x.y.z
```
...
When a new version of the Substrait specification is released, automation generates ANTLR bindings for that version and pushes them to GitHub with a tag formatted like `java/substrait-antlr/vx.y.z`. The automation then publishes that code to Maven Central.
...
```sh
cd java
./gradlew :substrait-antlr:build -Pversion=x.y.z
```
