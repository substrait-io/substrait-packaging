...
Versions of this artifact correspond to Substrait [releases](https://github.com/substrait-io/substrait/releases). `vx.y.z` of `io.substrait:protobuf` will contain code generated from `vx.y.z` of the [substrait repository](https://github.com/substrait-io/substrait).
...
When a new version of the Substrait specification is released, automation generates protobuf bindings for that version and pushes them to GitHub with a tag formatted like `java/protobuf/vx.y.z`. The automation then publishes that code to Maven Central.
...
```sh
cd java
./gradlew :protobuf:build -Pversion=x.y.z
```
